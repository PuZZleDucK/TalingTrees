...
## Tree Name Rating Scale
Tree names are rated by the LLM before being accepted. Five traits are scored from 1–10:

* **Familiarity** – Does it look like a human name?
* **Pronounceability** – Can you say it in one try?
* **Fantasy Flair** – Does it sparkle with magic or dragons?
* **Uniqueness** – Not too ordinary, not too bonkers.
* **Structure** – Two or three parts (first + middle + last).

Each trait follows this rough guide:
...
Uniqueness
  * 1 – "Michael John" (ultra-common)
  * 3 – "Sophia Grace" (still everywhere)
  * 5 – "Luna Maple" (noticeable twist)
...
Score each trait, add them together and divide by five for a final 1–10 rating. The tree name is accepted only if this rating meets the threshold set in `config/llm.yml`.
...
[ ] update tree naming prompt to align with rating guidance
[ ] update system prompt to be generated by an llm with guidance based on name, relations and other data
